...
use crate::lfn::{ShortName, LFN_PART_LEN, LongNameBuilder};
...
pub struct DirEntry {
    ...
    pub offset: u64,
    pub disk: Mutex<BlkCacheManager>,
...
impl DirEntry {
    ...
    fn write_entry(&mut self, name: &str, raw_entry: DirFileEntry) -> Result<DirEntry, Error<()>> {
        ...
        let offset = 0;
        let disk = Mutex::new(BlkCacheManager::new());
    ...
    pub fn create_file(&mut self, path: &str) -> Result<Inode, Error<()>> {
        ...
        match self.check_for_existence(name, Some(false))? {
    ...
    pub fn ls(&mut self) {
        ...
        for dir in self {
            if dir.is_dir() {
                info!("Dir: {}", dir.file_name());
            } else {
                info!("DirFile: {}", dir.file_name());
            }
        }
    }
        ...
        loop {
            ...
            match raw_entry {
                DirEntryData::File(data) => {
                    ...
                    let mut blk = BlkCacheManager::new();
                    let offset = match data.first_cluster() {
                        Some(x) => cluster_to_offset(x),
                        None => 0,
                    };

                    blk.seek(SeekFrom::Start(offset))?;
                    let disk = Mutex::new(blk);
                    return Ok({
                        Some(Self {
                            dir_entry: data,
                            offset,
